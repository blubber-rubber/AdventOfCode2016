...
states = []
heappush(states, (0, 0, tuple(starting_state)))
...
while states and not all_top_floor(states[0][2]):
    n_moves, elevator_floor, floors = heappop(states)
    if n_moves <= visited[(elevator_floor, floors)]:
        for new_floor, new_state in get_next_states(elevator_floor, floors):
            if n_moves + 1 < visited[(new_floor, new_state)]:
                visited[(new_floor, new_state)] = n_moves + 1
                heappush(states, (n_moves + 1, new_floor, new_state))
